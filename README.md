...
#include "cppgen/PingService.h" // 这里需要修改模版引用位置
...
编译 `server.cpp` 与 `client.cpp`，按照网络上某些过时的教程操作可能会出现报错，这里已经将编译 `shell` 命令脚本列出，直接执行 `compile_cpp_client.sh` 与 `compile_cpp_server.sh` 即可。

```bash
chmod +x compile_cpp_client.sh compile_cpp_server.sh
./compile_cpp_server.sh
...
# terminal#1
./server
# You will see `Starting cpp server...`
# terminal#2
./client
# You will see `ping(): ping` and `say(): Hello from cpp` at terminal#2
